...
class PuzzleCaptchaSolver:
    ...
    def remove_whitespace(self, image):
        """
        Loại bỏ khoảng trắng xung quanh ảnh bằng cách cắt ảnh theo vùng chứa nội dung chính.
        
        :param image: Ảnh đầu vào (numpy array).
        :return: Ảnh đã được cắt bỏ khoảng trắng.
        """
        min_x, min_y, max_x, max_y = 255, 255, 0, 0
        rows, cols, channel = image.shape
        for x in range(1, rows):
            for y in range(1, cols):
                if len(set(image[x, y])) >= 2:  # Kiểm tra nếu pixel không phải là màu đồng nhất
                    min_x, min_y = min(x, min_x), min(y, min_y)
                    max_x, max_y = max(x, max_x), max(y, max_y)
        if min_x >= max_x or min_y >= max_y:
            return image  # Trả về ảnh gốc nếu không tìm thấy vùng nội dung
        whitespace_removed_img = image[min_x:max_x, min_y:max_y]
        return whitespace_removed_img

    def apply_edge_detection(self, img):
        """
        Áp dụng edge detection để làm nổi bật các cạnh trong ảnh.
        
        :param img: Ảnh đầu vào (numpy array).
        :return: Ảnh với các cạnh được làm nổi bật (màu trắng trên nền đen).
        """
    ...
    def extract_thin_outline(self, image):
        """
        Tách viền ngoài mảnh ghép, tạo đường mảnh, và cắt ảnh theo kích thước của viền.
    ...
    def find_position_of_slide(self, slide_pic, background_pic, draw_on_image, draw_rectangle=False):
        ...
        :param background_pic: Ảnh background đã xử lý (chỉ còn viền).
        :param draw_on_image: Ảnh để vẽ hình chữ nhật (ở đây là ảnh đã xử lý, chỉ có viền).
        ...
        if draw_rectangle and self.output_image_path:
            cv2.rectangle(draw_on_image, max_loc, br, (0, 0, 255), 2)
    ...
    def evaluate_all_gaps(self, background_pic, draw_on_image):
        ...
        # Sau khi tìm được vị trí tốt nhất, vẽ một hình chữ nhật duy nhất trên ảnh đã xử lý
        if best_gap_path:
    ...
    def discern(self):
        ...
        # Xử lý ảnh background: áp dụng edge detection và loại bỏ khoảng trắng
        ...
        # Đánh giá vị trí ghép, vẽ hình chữ nhật trên ảnh đã xử lý (chỉ có viền)
